	...
	Pages.prototype = {
		...
		getNext: function () {
			...
			if(k.current >= k.count-1) {
				...
				return k.lis.item(k.current+1);
			}
		},
		getPre: function () {
			var k = this;
			if(k.current <= 0) {
				return k.lis.item(k.len-1);
			} else {
				return k.lis.item(k.current - 1);
			}
		},
        ...
        touchstart: function(e) {
            ...
            if (window.PointerEvent || window.MSPointerEvent) {
                var sleft = this.getCur().style.msTransform || this.getCur().style.transform;
                this.left = this.getLeft(sleft, "x");
            } else {
                this.left = this.getLeft(this.getCur().style.webkitTransform, "x");
            }
            e.preventDefault();
		},
        touchmove: function(e) {
		   ...
		   this.setX(this.getNext(), this.w+this.left+lenX, 0);
		   this.setX(this.getPre(), this.left+lenX-this.w, 0);
		   this.setX(this.getCur(), this.left+lenX, 0);
        },
        touchend: function(e) {
		   e.preventDefault();
		   var k = this;
        ...
        go: function(idx) {
            ...
            this.current = idx;
            len = -(idx * this.w);
			this.setX(this.getNext(), this.w+len, this.interval);
		    this.setX(this.getPre(), len-this.w, this.interval);
		   this.setX(this.getCur(), len, this.interval);
        },
